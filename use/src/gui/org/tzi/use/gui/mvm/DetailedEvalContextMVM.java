 ...
 *
 */
public class DetailedEvalContextMVM extends EvalContextMVM {
	private Stack<EvalNodeMVM> fNodeStack = new Stack<EvalNodeMVM>();
	private EvalNodeMVM fRootNode;
	
	public DetailedEvalContextMVM(MSystemState preState,
	...
	void enter(ExpressionMVM expr) {
		super.enter(expr);
		EvalNodeMVM n = new EvalNodeMVM(varBindings());
		n.setExpression(expr);
		fNodeStack.push(n);
	}

	@Override
	void exit(ExpressionMVM expr, Value result) {
		super.exit(expr, result);
	    EvalNodeMVM n = (EvalNodeMVM) fNodeStack.pop();
	    n.setResult(result);
